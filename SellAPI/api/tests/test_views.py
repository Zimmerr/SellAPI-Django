...
class GetAllVendedoresTest(TestCase):
  ...
  def test_get_all_vendedores(self):
    ...
    responseSerializer = VendedorSerializer(response_data, many=True)
...
class GetVendedorTest(TestCase):
  ...
  def test_get_vendedor_valido(self):
    ...
    responseSerializer = VendedorSerializer(response_data)
    self.assertEqual(responseSerializer.data, serializer.data)
    self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class GetVendedorAniversarianteTest(TestCase):
  def setUp(self):
    Vendedor.objects.create(
      nome='Thiago Neves', data_nasc='1975-05-12')
    Vendedor.objects.create(
      nome='Gerson', data_nasc='1995-05-27')
    Vendedor.objects.create(
      nome='Claudinho', data_nasc='1991-11-01')

  def test_get_aniversariantes_maio(self):
    mes_certo = '05'
    response = client.get('/api/vendedores/aniversariantes/' + mes_certo)
    response_data = json.loads(response.content)['data']

    vendedores = Vendedor.objects.filter(data_nasc__month= mes_certo)
    serializer = VendedorSerializer(vendedores, many=True)
    responseSerializer = VendedorSerializer(response_data, many=True)
    self.assertEqual(responseSerializer.data, serializer.data)
    self.assertEqual(response.status_code, status.HTTP_200_OK)

  def test_get_aniversariantes_mes_invalido(self):
    response = client.get('/api/vendedores/aniversariantes/999')
    self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)